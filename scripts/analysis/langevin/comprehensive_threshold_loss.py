"""Comprehensive 4-parameter Langevin sampling for threshold algorithm using precise loss function.

This script optimizes:
- Threshold (threshold)
- Derivative threshold (derivative_threshold)
...
THRESHOLD_VALUES = np.linspace(90, 300, 40)  # Threshold: 90 to 300
DERIVATIVE_THRESHOLD_VALUES = np.linspace(0, 100, 40)  # Derivative threshold: 0 to 100
...
PARAM_BOUNDS = {
    "threshold": (90.0, 300.0),
    "derivative_threshold": (0.0, 100.0),
...
def compute_loss_for_parameter_combination_wrapper(
    args: tuple[np.ndarray, list[int], float, float, float, float],
) -> tuple[float, float, float, float, float]:
    """Wrapper function for parallel processing of parameter combinations.
    
    Args:
        args: Tuple of (concatenated_data, concatenated_markers, threshold, 
                       derivative_threshold, min_flight_time, max_flight_time)
        
    Returns:
        Tuple of (threshold, derivative_threshold, min_flight_time, max_flight_time, loss)
    """
    concatenated_data, concatenated_markers, threshold, derivative_threshold, \
        min_flight_time, max_flight_time = args
    ...
    return threshold, derivative_threshold, min_flight_time, max_flight_time, loss
...
def print_optimized_results(results: list[dict], top_n: int = 20) -> None:
    ...
    print(f"{'='*100}")
    print(f"{'Rank':<6} {'Threshold':<12} {'Deriv':<10} {'Min Time':<10} {'Max Time':<10} {'Loss':<10}")
    print(f"{'-'*100}")
    ...
    for i, result in enumerate(results[:top_n], 1):
        print(
            ...
            f"{result['derivative_threshold']:<10.4f} "
    ...
    if results:
        ...
        print(f"  Derivative threshold: {best['derivative_threshold']:.6f}")
...
def random_initial_parameters() -> dict[str, float]:
    ...
    """
    return {
        "threshold": random.uniform(*PARAM_BOUNDS["threshold"]),
        "derivative_threshold": random.uniform(*PARAM_BOUNDS["derivative_threshold"]),
...
def perturb_parameters(
    ...
    if step_sizes is None:
        ...
        step_sizes = {
            "threshold": (PARAM_BOUNDS["threshold"][1] - PARAM_BOUNDS["threshold"][0]) * 0.05,
            "derivative_threshold": (PARAM_BOUNDS["derivative_threshold"][1] - PARAM_BOUNDS["derivative_threshold"][0]) * 0.05,
...
def langevin_sampling(
    ...
    current_loss = compute_loss_for_parameter_combination(
        ...
        current_params["derivative_threshold"],
    ...
    all_results = [{
        ...
        "derivative_threshold": float(current_params["derivative_threshold"]),
    ...
    for iteration in range(1, n_iterations + 1):
        ...
        proposed_loss = compute_loss_for_parameter_combination(
            ...
            proposed_params["derivative_threshold"],
        ...
        all_results.append({
            "threshold": float(proposed_params["threshold"]),
            "derivative_threshold": float(proposed_params["derivative_threshold"]),
    ...
    best_result = {
        ...
        "derivative_threshold": float(best_params["derivative_threshold"]),
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive parameter optimization for threshold algorithm"
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    print("COMPREHENSIVE 4-PARAMETER OPTIMIZATION (THRESHOLD ALGORITHM)")
    ...
    if args.method in ["langevin", "both"]:
        ...
        print(f"  Threshold:           {langevin_best['threshold']:.6f}")
        print(f"  Derivative threshold: {langevin_best['derivative_threshold']:.6f}")
        ...
        langevin_output = OUTPUT_DIR / "comprehensive_threshold_loss_langevin_results.json"
