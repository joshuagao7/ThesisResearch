"""Comprehensive 5-parameter grid search for hybrid algorithm using precise loss function.

This script sweeps through:
- Takeoff threshold (takeoff_threshold)
- Landing derivative threshold (landing_derivative_threshold)
...
from jump_detection.algorithms.hybrid import (
    HybridParameters,
...
from jump_detection.types import DetectionResult, Jump, HybridNames

# Parameter ranges for grid search
TAKEOFF_THRESHOLD_VALUES = np.linspace(100, 400, 20)  # Takeoff threshold: 100 to 400
LANDING_DERIVATIVE_THRESHOLD_VALUES = np.linspace(0, 100, 20)  # Landing derivative threshold: 0 to 100
...
PARAM_BOUNDS = {
    "takeoff_threshold": (100.0, 400.0),
    "landing_derivative_threshold": (0.0, 100.0),
...
def compute_precise_jumps_from_concatenated(
    ...
    Args:
        ...
        result: DetectionResult from hybrid pipeline
...
def compute_loss_for_parameter_combination(
    ...
    Args:
        ...
        takeoff_threshold: Takeoff threshold parameter
        landing_derivative_threshold: Landing derivative threshold parameter
    ...
    # Run hybrid pipeline directly on concatenated data
    signals, jumps, metadata = _run_hybrid_pipeline(concatenated_data, params)
    ...
    result = DetectionResult(
        ...
        pooled_data=signals[HybridNames.POOLED.value],
        signals=signals,
        jumps=jumps,
        metadata=metadata,
    )
...
def compute_loss_for_parameter_combination_wrapper(
    ...
    Args:
        args: Tuple of (concatenated_data, concatenated_markers, takeoff_threshold, 
                       landing_derivative_threshold, in_air_threshold, min_flight_time, max_flight_time)
        
    Returns:
        Tuple of (takeoff_threshold, landing_derivative_threshold, in_air_threshold, 
                 min_flight_time, max_flight_time, loss)
    """
    concatenated_data, concatenated_markers, takeoff_threshold, landing_derivative_threshold, \
    ...
    return takeoff_threshold, landing_derivative_threshold, in_air_threshold, min_flight_time, max_flight_time, loss
...
def comprehensive_grid_search(
    ...
    all_combinations = list(product(
        TAKEOFF_THRESHOLD_VALUES,
        LANDING_DERIVATIVE_THRESHOLD_VALUES,
    ...
    valid_combinations = [
        (takeoff, landing, in_air, min_time, max_time)
        for takeoff, landing, in_air, min_time, max_time in all_combinations
        if min_time < max_time
    ]
    
    total = len(valid_combinations)
    print(f"Total parameter combinations to test: {total:,}")
    print(f"  Takeoff threshold: {len(TAKEOFF_THRESHOLD_VALUES)} values")
    print(f"  Landing derivative threshold: {len(LANDING_DERIVATIVE_THRESHOLD_VALUES)} values")
    ...
    parameter_combinations = [
        (concatenated_data, concatenated_markers, float(takeoff), float(landing),
         float(in_air), float(min_time), float(max_time))
        for takeoff, landing, in_air, min_time, max_time in valid_combinations
    ]
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_combo):
            try:
                takeoff_threshold, landing_derivative_threshold, in_air_threshold, \
                    min_flight_time, max_flight_time, loss = future.result()
                
                results.append({
                    "takeoff_threshold": float(takeoff_threshold),
                    "landing_derivative_threshold": float(landing_derivative_threshold),
...
def print_optimized_results(results: list[dict], top_n: int = 20) -> None:
    ...
    print(f"{'='*100}")
    print(f"{'Rank':<6} {'Takeoff':<10} {'Landing':<10} {'In Air':<10} {'Min Time':<10} {'Max Time':<10} {'Loss':<10}")
    print(f"{'-'*100}")
    
    for i, result in enumerate(results[:top_n], 1):
        print(
            f"{i:<6} "
            f"{result['takeoff_threshold']:<10.4f} "
            f"{result['landing_derivative_threshold']:<10.4f} "
    ...
    if results:
        best = results[0]
        print(f"\nBEST RESULT:")
        print(f"  Takeoff threshold:         {best['takeoff_threshold']:.6f}")
        print(f"  Landing derivative threshold: {best['landing_derivative_threshold']:.6f}")
...
def random_initial_parameters() -> dict[str, float]:
    ...
    """
    return {
        "takeoff_threshold": random.uniform(*PARAM_BOUNDS["takeoff_threshold"]),
        "landing_derivative_threshold": random.uniform(*PARAM_BOUNDS["landing_derivative_threshold"]),
...
def perturb_parameters(
    ...
    if step_sizes is None:
        ...
        step_sizes = {
            "takeoff_threshold": (PARAM_BOUNDS["takeoff_threshold"][1] - PARAM_BOUNDS["takeoff_threshold"][0]) * 0.05,
            "landing_derivative_threshold": (PARAM_BOUNDS["landing_derivative_threshold"][1] - PARAM_BOUNDS["landing_derivative_threshold"][0]) * 0.05,
...
def langevin_sampling(
    ...
    current_loss = compute_loss_for_parameter_combination(
        concatenated_data,
        concatenated_markers,
        current_params["takeoff_threshold"],
        current_params["landing_derivative_threshold"],
    ...
    all_results = [{
        "takeoff_threshold": float(current_params["takeoff_threshold"]),
        "landing_derivative_threshold": float(current_params["landing_derivative_threshold"]),
    ...
    for iteration in range(1, n_iterations + 1):
        ...
        proposed_loss = compute_loss_for_parameter_combination(
            concatenated_data,
            concatenated_markers,
            proposed_params["takeoff_threshold"],
            proposed_params["landing_derivative_threshold"],
        ...
        all_results.append({
            "takeoff_threshold": float(proposed_params["takeoff_threshold"]),
            "landing_derivative_threshold": float(proposed_params["landing_derivative_threshold"]),
    ...
    best_result = {
        "takeoff_threshold": float(best_params["takeoff_threshold"]),
        "landing_derivative_threshold": float(best_params["landing_derivative_threshold"]),
...
def save_results_to_json(results: list[dict], output_path: Path) -> None:
    ...
    output_data = {
        ...
        "parameter_ranges": {
            "takeoff_threshold": {
                "min": float(np.min(TAKEOFF_THRESHOLD_VALUES)),
                "max": float(np.max(TAKEOFF_THRESHOLD_VALUES)),
                "count": len(TAKEOFF_THRESHOLD_VALUES),
            },
            "landing_derivative_threshold": {
                "min": float(np.min(LANDING_DERIVATIVE_THRESHOLD_VALUES)),
                "max": float(np.max(LANDING_DERIVATIVE_THRESHOLD_VALUES)),
                "count": len(LANDING_DERIVATIVE_THRESHOLD_VALUES),
            },
            "in_air_threshold": {
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive parameter optimization for hybrid algorithm"
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    print("COMPREHENSIVE 5-PARAMETER OPTIMIZATION (HYBRID ALGORITHM)")
    ...
    for data_file in DATA_FILES:
        ...
        # Load ground truth annotations - skip if not available
        annotations = load_annotations(file_path)
        
        if annotations is None or not annotations.markers:
            print(f"  ⚠ Skipping {participant} – {file_label}: No annotations found")
            continue
        
        ground_truth_markers = annotations.markers
        
        if not ground_truth_markers or len(ground_truth_markers) == 0:
            print(f"  ⚠ Skipping {participant} – {file_label}: No markers in annotations")
            continue
            print(f"\n{'='*70}")
    ...
    if args.method in ["grid", "both"]:
        ...
        output_path = OUTPUT_DIR / "comprehensive_hybrid_loss_grid_results.json"
    ...
    if args.method in ["langevin", "both"]:
        ...
        print(f"  Takeoff threshold:         {langevin_best['takeoff_threshold']:.6f}")
        print(f"  Landing derivative threshold: {langevin_best['landing_derivative_threshold']:.6f}")
        ...
        langevin_output = OUTPUT_DIR / "comprehensive_hybrid_loss_langevin_results.json"
    ...
    if args.method == "both" and all_results:
        ...
        combined_output = OUTPUT_DIR / "comprehensive_hybrid_loss_combined_results.json"
