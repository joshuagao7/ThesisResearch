"""Threshold-based summary plots and interactive viewer - uses all DEFAULT_DATA_FILES."""
...
from jump_detection.algorithms.threshold import process_all_threshold_participants
...
def create_summary_plot() -> Sequence:
    """Create summary plot for threshold algorithm using all DEFAULT_DATA_FILES."""
    data_files = get_all_data_files()
    print(f"Processing {len(data_files)} data files for threshold algorithm...")
    results = process_all_threshold_participants(data_files=data_files, save_windows=False)

    fig, axes = plt.subplots(len(results), 1, figsize=(20, 15), sharex=True)
    fig.suptitle("Final Processed Jump Regions - All Participants (Threshold Algorithm)", fontsize=16)
    ...
    save_path = project_root / "results" / "plots" / "summary_plot_threshold.png"
...
def find_missed_jumps(
    ...
    Returns:
        Tuple of:
        - List of marker indices that were not contained in any detected jump (false negatives)
        - List of detected jumps that don't contain any marker (false positives)
    """
    ...
    # Find markers that don't fall within any detected jump (false negatives)
    ...
    # Find detected jumps that don't contain any marker (false positives)
    for jump in detected_jumps:
...
def create_error_diagnostic_plots(results: Sequence) -> None:
    ...
    # Collect all false negatives and false positives
    ...
    if all_false_negatives:
        ...
        fig.suptitle(
            f"False Negatives (Missed Ground Truth Markers) - Threshold Algorithm\n"
    ...
    if all_false_positives:
        ...
        fig, axes = plt.subplots(rows, cols, figsize=(5 * cols, 4 * rows), sharey=True)
        fig.suptitle(
            f"False Positives (Detected Jumps Without Ground Truth) - Threshold Algorithm\n"
        ...
        for idx, (file_path, jump, signal) in enumerate(all_false_positives):
            ...
            ax.axvspan(
                jump.start - start_frame,
                jump.end - start_frame,
                alpha=0.3,
                color=PRIMARY_ORANGE,
                label="False Positive Jump",
            )
            ax.axvline(
                jump.center - start_frame,
                color=PRIMARY_ORANGE,
                linestyle="--",
                linewidth=2,
                alpha=0.9,
    ...
    print(f"\n{'='*80}")
    print(f"Error Summary - Threshold Algorithm")
    print(f"{'='*80}")
...
class InteractiveJumpViewer:
    ...
    def _create_interactive_plot(self) -> None:
        ...
        self.fig.suptitle(
            "Interactive Jump Viewer (Threshold) - Use ← → arrows to navigate, ESC to close",
            fontsize=14,
        )
        self._update_plot()
        plt.show()
...
def create_interactive_jump_viewer() -> None:
    """Create interactive jump viewer for threshold algorithm."""
    print("Loading data for interactive jump viewer (Threshold)...")
    data_files = get_all_data_files()
    results = process_all_threshold_participants(data_files=data_files, save_windows=False)
...
if __name__ == "__main__":
    print("Creating threshold summary plot...")
    create_summary_plot()
    print("\nStarting threshold interactive jump viewer...")
